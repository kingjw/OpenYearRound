...
(function (factory) {
  ...
  var swipe = function (options) {
    ...
    $target.addClass('recopick-swipe-target').css({
    ...
    if (options.infinite) {
      $target.clone().attr('aria-hidden', true)
             .addClass('recopick-swipe-target-clone').css('left', $target.width())
             .insertBefore($target);
      $target = $target.parent().find('.recopick-swipe-target');
    }
    ...
    $el.on('dragleft', function (e, velocityX, deltaX) {
      ...
      if (Math.abs(velocityX) > 5 && !(p === total && !options.infinite)) {
        ...
        swiped = true;
        curr_px = 0;
        $target.removeClass('recopick-swipe-disabled-transition');
        swipeNext();
        return;
      }
      var l = deltaX;
      ...
      $target.addClass('recopick-swipe-disabled-transition');
  ...
  $.fn.recopickSwipeEvent = function () {
    var $el = $(this),
        mdown = false,
        mdirection = null,
  ...
  $.fn.recopickSwipe = function (options) {
    return this.each(function () {
      $(this).recopickSwipeEvent();
